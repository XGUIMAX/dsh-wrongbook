 ...
 * 所以这里把「先查、后归档、归档完回报一句」明确写下来。
 */
const WRONGBOOK_PROMPT = [
  '【错题库】本机装了 dsh-wrongbook，错题的唯一归档位置是它（面板入口：设置 → 错题库）。',
  '',
  '调试人物卡、排查 DSH / Tavern / 插件问题时：',
  '',
  '1. 动手前先查：`wrongbook_lookup { card: "<卡名或主题>" }`。结果按「① 本分类 → ② 其它错题 → ③ 跨卡查询」三段返回，前一段优先看。',
  '2. 解决之后归档：`wrongbook_record { card, title, symptom, cause, fix, scope, tags }`，现象、根因、证据、处理、防坑都写进去。分类名不认识会自动建；`card` 也可以填「卡片更新器」这类主题名。',
  '3. 库里已有同一条：`wrongbook_update { id, status, cause, fix }` 补根因或标记已修复。',
  '4. 归档完必须在回复里回报：写明「已归档到错题库」＋分类名和条目标题。用户靠这句话确认归档真的发生了。',
  '5. 不要再把错题写进 md 文件：`materials/错题库.md`、技能里的 `references/` 都已经迁走，写进去等于没归档 —— 面板查不到，下次调试也检索不到。',
].join('\n')

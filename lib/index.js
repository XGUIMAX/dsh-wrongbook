...
function parseRefluxFile(text) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (next.startsWith('<!-- wrongbook:') || next.startsWith('> 错题库回流：') || after.startsWith('> 错题库回流：')) {
      starts.push(i)
    }
  }
  ...
  for (let k = 0; k < starts.length; k += 1) {
    ...
    const m = rest.match(/^<!-- wrongbook:([^|]+)\|([^|]*)\|([0-9a-f]+) -->[ \t]*\n?/)
    // 标记行后面那个空行也要去掉：留着的话它会被算进正文，指纹就永远对不上，
    // 于是每段都被误判成"手工改过"，整个更新机制静默失效。
    const rawBody = m ? rest.slice(m[0].length) : rest
    sections.push({

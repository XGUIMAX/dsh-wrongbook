...
function lookup(request) {
  ...
  for (const item of scored) {
    ...
    if (key === cardKey) own.push(item.entry)
    else if (groupOf(key) === 'other') other.push(item.entry)
    else cross.push(item.entry)
    if (own.length + other.length + cross.length >= limit * 3) break
  }
 ...
 * 把用户写的名字落到一个分类上。
 *
 * 分层匹配，越精确越优先。**模糊命中多个时不猜** —— 落回兜底分类，而不是按
 * 对象键的遍历顺序抓第一个：抓第一个的代价是同一句话在不同时刻可能落到不同分类，
 * 那比查不到更难查。命中多个时优先原版卡（`X MVU版本` 是它的派生），其次名字最短的，
 * 最后按名字排序 —— 无论落在哪一档，同一个输入永远得到同一个结果。
 */
function resolveCardKey(input) {
  const raw = str(input).trim()
  if (!raw) return GENERAL_KEY
  if (db.cards[raw]) return raw
  if (isGeneralAlias(raw)) return GENERAL_KEY

  const base = path.basename(raw).replace(/\.json$/i, '')
  if (!base) return GENERAL_KEY
  ...
  if (!fuzzy.length) return GENERAL_KEY
  if (fuzzy.length === 1) return fuzzy[0]

  // 多个命中：只有它们本是同一张卡的不同版本时才敢挑，取原版
  //（`X MVU版本` 是 `X` 的派生）。否则不猜 —— 猜错的代价是把另一张卡的结论
  // 递到手上，那比查不到更难发现。
  const stripped = (k) => baseOf(k).replace(/\s*MVU\s*版本$/i, '')
  const families = new Set(fuzzy.map(stripped))
  if (families.size !== 1) return GENERAL_KEY
  return fuzzy.find((k) => str(db.cards[k].kind) !== 'mvu') || fuzzy[0]
}
...
function addEntry(payload) {
  ...
  let cardKey = resolveCardKey(wanted)
  let createdBucket = ''
...
function renderLookup(result, options) {
  ...
  const note = (list) =>
    list.length > cap ? `（共 ${list.length} 条，这里先列 ${cap} 条；带上关键词可以筛）` : ''

  const inOther = result.cardGroup === 'other'
  const parts = [`【错题库检索】当前分类=${result.cardName} · 全库 ${result.scanned} 条`, '']
  parts.push(`${inOther ? '① 本分类错题库' : '① 本卡错题库'}（${result.cardName}）· ${result.own.length} 条`)
  parts.push(result.own.length ? result.own.map((e) => renderEntry(e, false)).join('\n') : '（没有记录）')

  parts.push('')
  parts.push(`② 其它错题（不挂在卡上的问题）· ${result.other.length} 条`)
  parts.push(
    result.other.length
      ? [clip(result.other).map((e) => renderEntry(e, true)).join('\n'), note(result.other)].filter(Boolean).join('\n')
      : '（没有记录）',
  )
  ...
  parts.push(
    result.cross.length
      ? [clip(result.cross).map((e) => renderEntry(e, true)).join('\n'), note(result.cross)].filter(Boolean).join('\n')
      : '（没有记录）',
  )

...
function refluxToSkill(payload) {
  ...
  if (!blocks.length) {
    // total 是写完之后 skill 里一共有多少条 —— 回执要报这个数，用户才知道东西真的落地了。
    return { skill: name, file: targetFile, written: 0, skipped, bytes: before.length, created: false, total: seen.size }
  }
...
function refluxNote(brief) {
  if (!brief) return '\n（自动同步没开，这条只存在错题库里。）'
  return `\n自动同步：已写进 skill「${brief.skill}」，本次新增 ${brief.written} 条（跳过 ${brief.skipped} 条），该 skill 现在共 ${brief.total} 条。`
}

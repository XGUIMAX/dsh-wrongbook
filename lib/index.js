...
function listSkills() {
  ...
  const scan = (root, builtin) => {
    ...
    for (const dirent of entries) {
      ...
      let references = []
      try {
        references = fs.readdirSync(path.join(dir, 'references')).filter((n) => n.endsWith('.md'))
      } catch {
        /* 没有 references 目录 */
      }
      out.push({ name: dirent.name, dir, builtin, description: skillDescriptionOf(skillFile), references })
    }
  }
...
function createUserSkill(payload) {
  ...
  const description =
    str(payload && payload.description).trim() || `${name}：把错题库里调出来的结论沉淀下来，做同类事情前先读一遍。`
  const body = [
    '---',
    `name: ${name}`,

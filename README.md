...
弹窗里填三件事：目标 skill、范围（全部 / 当前分类 / 未解决的）、目标文件（默认 `references/错题库回流.md`）。每条按**现象 / 根因 / 修法 / 证据**铺开，带上分类、范围和状态。

**只增不改**：按标题去重，文件里已有同名的不再写第二遍，你手工改过的内容也不会被覆盖。所以同一个目标可以反复回流，新调的坑追加在末尾。

**只能写你自己的 skill。** 内置 skill（`card-to-mvu`、`debug-card` 这些）在程序目录里，Tavern 更新会整份覆盖它们 —— 写了也留不住。弹窗把它们一并列出来但标成写不了，并说明原因；要往那个语境里沉淀，先在弹窗里**新建一个自己的 skill**（名字用小写字母、数字和连字符），它会连带建好 `SKILL.md` 和那份回流文件。

两条硬边界：目标文件不许跑出 skill 目录（`..` 会被拦），skill 名也不许带路径。

**新建 skill 时把简介写实。** `description` 是 Agent 判断「要不要自动加载这个 skill」的**唯一依据** —— 光有名字加一句"这是个错题库"，它不知道自己该在什么时候想起来。留空时插件会给一句通用的默认值，但那只能算占位；写清场景（比如"排查 MVU 转换后的功能缺失时读一遍"）它才会在合适的时机被读到。弹窗里有这一栏。
...
```bash
node verify/host.mjs      # 193 项：扫描、分组、归类、卡名解析、三段检索、卡内脚本、安装自检、skill 回流、路由、工具、备份
node verify/client.mjs    # 84 项：注册接线、字典一致性、四个页签与两个弹窗、三段渲染、禁用 API 静态检查

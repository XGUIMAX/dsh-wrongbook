...
`card` 的解析是分层的：完整路径 → 文件基名 → 分类显示名完全相等 → 名字包含。**模糊命中多张不同的卡时不猜**，落到「通用 / 未归类」—— 猜错的代价是把另一张卡的结论递到手上，那比查不到更难发现。命中同一张卡的不同版本（`X` 与 `X MVU版本`）时取原版。
...
```bash
node verify/host.mjs      # 123 项：扫描、分组、归类、卡名解析、三段检索、路由、工具、自检、备份、目录浏览
node verify/client.mjs    # 62 项：注册接线、字典一致性、三个页签与浏览弹窗、三段渲染、禁用 API 静态检查
```
